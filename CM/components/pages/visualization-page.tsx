...
export default function VisualizationPage() {
  ...
  const fetchVisualizations = async (projectId: string | null) => {
    ...
    try {
      ...
      let url = `/api/visualizations?diagram_type=flowchart&limit=20`
      if (projectId) {
  ...
  return (
    <div className="h-full p-8 space-y-6 overflow-auto bg-slate-50/50">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-slate-800">Process Visualization</h1>
          <p className="text-slate-500 mt-1">
            {currentProject 
              ? `Life cycle flowcharts for ${currentProject.name}` 
              : visualizations.length > 0 
                ? "Showing all available flowcharts" 
                : "Select a project or run an analysis to generate flowcharts"}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-[400px] bg-white rounded-2xl border border-dashed border-slate-200">
          ...
          <p className="text-slate-500 font-medium">No flowcharts available</p>
